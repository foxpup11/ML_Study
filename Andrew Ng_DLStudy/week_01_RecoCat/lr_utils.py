...
import h5py
def load_dataset():
    train_dataset = h5py.File('F:/ML_practice/Andrew Ng_DLStudy/week_01_RecoCat/datasets/train_catvnoncat.h5','r')
    train_set_x_orig = np.array(train_dataset["train_set_x"][:]) #获取训练集特征x
    train_set_y_orig = np.array(train_dataset["train_set_y"][:]) #获取训练集标签y

    test_dataset = h5py.File('F:/ML_practice/Andrew Ng_DLStudy/week_01_Recocat/datasets/test_catvnoncat.h5','r')
